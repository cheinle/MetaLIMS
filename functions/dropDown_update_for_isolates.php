...
function dropDown_update_for_isolates($select_name,$table_name,$field_name,$select_id,$s_field_name,$sample_name){ #send also the query name?, always based on sample name
			...
			if($check == 'true' && $check2 == 'true'){
				...
				$query1 = "SELECT $p_s_field_name FROM isolates WHERE sample_name = '$p_sample_name'";
				...
				while($row = mysqli_fetch_assoc($result)) {
					...
					$name = trim($name);
					$name1 = trim($name1);
					//echo '<option value="'.$id.'">name'.$name1.'test</option>';
					//echo '<option value="'.$id.'">id'.$id.'tid</option>';
					if($id == $name1){
?>
