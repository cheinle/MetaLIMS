					...
					if($visible_check == '1'){
						if ((isset($_GET['submit']) && $submitted != 'true') || (isset($_GET['copy'])))   {
							if(isset($_SESSION['submitted']) && $_SESSION['submitted'] == 'false'){
									echo '<option value="<?php echo $id; ?>" <?php if((isset($_SESSION["$p_select_name"]) && $_SESSION["$p_select_name"] == "$id" )){ echo "selected";} ?>><?php echo "$name" ?></option>';
							}
							else{
								echo '<option value="<?php echo $id; ?>" <?php if((isset($_GET["$p_select_name"]) && $_GET["$p_select_name"] == "$id" )){ echo "selected";} ?>><?php echo "$name" ?></option>';
							}
						}
						else{
